<!DOCTYPE html>
<!DOCTYPE html>
<html lang="fr">

<head>
    <!-- En-tête de la page -->
    <link type="text/css" rel="stylesheet" href="../../CSS_files/FOOTER.css?t=<? echo time(); ?>" media="all">
    <!--<link rel="stylesheet" href="P_Accueil_Payen.css" />-->
    <meta charset="utf-8" />
    <title>WINK FOR INIFINITE MEASURES</title>
</head>
<body>

<footer>

    <div class="bloc_footer1">

        <div class ="signature">
            <p class="">  RÉALISÉ PAR</p>
            <img class="logo_WINK" src="../../../images/logowink.png" alt="Logo WINK" />
        </div>

        <div class="texte_footer">

            <h2>À Propos de nous</h2>
            <p>Wink est une jeune start-up française spécialisée dans la réalisation de tests psychotechniques</p>
            <p>Les tests sur cette plateforme ont été conçus pour Infinite Measures</p>
        </div>
        <a href="Apropos_gestionnaire.php" class="bouton_mail" alt="Envoyer un mail">INFINITE MEASURES & WINK</a>

    </div>

    <div class="trait_fin_footer"></div>


    <div class="bloc_footer2">
        <a href="CGU_Gestionnaire.php" class="conditions"><p>CONDITIONS</p></a>
        <a href="mentionsLegales_Gestionnaire.php" class="mentions"><p>MENTIONS LEGALES</p></a>
    </div>

</footer>

</body>
</html>
